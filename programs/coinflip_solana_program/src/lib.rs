...
pub mod solana_coinflip_game {
    ...
    pub fn create_coinflip(
        ...
        // Check if the game PDA has enough funds to cover a potential payout
        let required_balance = amount * 2; // The game needs to be able to pay double the bet
        if ctx.accounts.game_pda.lamports() < required_balance {
            return err!(HouseError::InsufficientFunds);
        }
        ...
        invoke(
            &transfer(
                ctx.accounts.player.to_account_info().key,
                ctx.accounts.game_pda.to_account_info().key,
                amount,
            ),
            &[
                ctx.accounts.player.to_account_info(),
    ...
    pub fn play_coinflip(
        ...
        let cpi_accounts = orao_solana_vrf::cpi::accounts::Request {
            payer: ctx.accounts.player.to_account_info(),
            network_state: ctx.accounts.config.to_account_info(),
            treasury: ctx.accounts.game_pda.to_account_info(),
            request: ctx.accounts.random.to_account_info(),
            system_program: ctx.accounts.system_program.to_account_info(),
        };
        let cpi_ctx = CpiContext::new(cpi_program, cpi_accounts);
    ...
    pub fn result_coinflip(
        ...
        match game_result {
            GameResult::PlayerWin => {
                // Transfer winnings from game PDA to player
                **ctx.accounts.game_pda.lamports.borrow_mut() -= coinflip.amount * 2;
                **ctx.accounts.player.lamports.borrow_mut() += coinflip.amount * 2;
                msg!("Player wins: {}", coinflip.player.to_string());
            }
            GameResult::HouseWin => {
                // Transfer bet from game PDA to house treasury
                **ctx.accounts.game_pda.lamports.borrow_mut() -= coinflip.amount;
                **ctx.accounts.house_treasury.lamports.borrow_mut() += coinflip.amount;
                msg!("House wins");
            }
            GameResult::Tie => {
...
#[error_code]
pub enum HouseError {
    #[msg("Not enough game PDA funds for payout")]
    InsufficientFunds,
}
